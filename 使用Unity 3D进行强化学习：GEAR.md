...
<img src="images/final3.gif" width="300">
<img src="images/final1.gif" width="300">
<img src="images/final2.gif" width="300">

SegNet：输入，基础事实，预测结果。

2.2 智能体的大脑brain

机器人认知的核心部分是大脑brain，这是agent的决策部分:在当前情况下应该采取什么行动。

这里尝试了这几种方法：

近端策略优化 - PPO是OpenAI开发的强化学习的当前最先进的策略梯度方法，通过与环境的交互采样数据，并使用随机梯度上升优化“替代”目标函数。

观察并克隆行为Behavioral Cloning from Observation - 这种方法将问题视为监督学习任务，我们“玩游戏”半小时，agent克隆我们的行为，通过这个过程agent学会了粗略的期望策略。
...
https://youtu.be/7YO3rsb3TwQ

agent的学习过程，可以看到经过24小时的训练后GEAR已经达到了不错的性能。

第一种方法是使用PPO算法训练agent，语义分割信息不是来自外部神经网络，而是使用Unity中的着色器生成，使用标签对对象进行分割。这意味着在训练期间agent可以快速得到物品类的可靠、无噪声的信息。

还利用Unity ML-Agents提供的两个改进措施：

**使用Recurrent Neural Networks进行内存增强** - agent决策时不仅考虑当前的RGBD输入，而且“记住”最后的n步输入，在做出决策时将此附加信息包含在推理中。实验表明这提高了GEAR在优先决策判断方面的能力，例如，当agent认识到有机会收集两个物品（更高的奖励）时，有时可能放弃单个物品的收集，并会最终返回收集那单个的垃圾。

**好奇心** - 当问题的外在信号非常稀疏时，agent没有足够的信息来确定正确的策略。我们可以赋予agent一种好奇心，每当发现与其当前掌握的知识相比起来比较让人吃惊或者非常规的东西时，就给予agent内部奖励。以此鼓励agent探索世界，更加具有“冒险”性。在我们的案例中很难说好奇心的影响是什么，但是我们注意到有几个时间点内部奖励在训练期间飙升并且显着改善了当前agent的策略。

我们花了好几天的课程训练时间来用PPO训练agent，有一种情况是如果一开始就将惩罚设置为高，agent会简单地只是转圈。可以通过在起始时让agent寻找主要目标来避免这种情况，agent明白它被鼓励做什么之后，就可以采取更严厉的惩罚形式进一步限制以微调它的行为。

3.2 PPO与语义分割网络

https://youtu.be/pr3DfmuhnfM

使用分割网络的情况下PPO模型的推断，算法不仅速度慢得多（使用最好的计算机仍有几毫秒的延迟），但agent的行为并不完全正确（SegNet的输出不如在Unity自定义着色器生成的图像精确）。

在现实生活中，我们无法使用在Unity 3D中自定义的着色器，这就是为什么我们应该训练自己的语义分割模型。模型准备就绪后，将SegNet嵌入Python API有两种方式：

**在训练brain时就用SegNet** - 这使得训练的时间效率非常低下，每个输入帧都需要由SegNet进行分割，这对于笔记本电脑来说计算成本太高。此外，agent的brain容易受到SegNet不完美输出的影响。优点是这种方法使得在Python API中实现SegNet非常简单。

**使用自定义着色器训练brain并在测试期间插入SegNet** - 这是更节省时间的解决方案，因为SegNet仅在推理时使用。而且这样就可以用自定义着色器的无噪声分割图像训练agent。缺点就是这需要使用Python API进行更多的工作，后期需要集成SegNet操作。

鉴于计算资源有限，而且应该使用无噪声数据训练brain，选择第二种方式。

3.3 行为克隆Behavioral Cloning

https://youtu.be/pCaLnJ8FL3I

这种方式中agent直接学习人类玩家，这其中的意思是，可以在半小时左右的时间内训练一个还行的agent，但它永远不会比人玩的更好。这种方法足以创建一个好的并且在某些任务中不是很优秀的agent（例如视频游戏AI，其中agent应该相对较弱以便我们可以享受玩游戏）。显然，使用这种方法训练对我们的目的来说还不够好。

3.4 启发式Heuristic

https://youtu.be/vDUq3du3f2Q

启发式算法需要较少的训练，并且非常善于识别真正类。而且一旦发现真正类，agent会收集它而不考虑它也可能无意中收集了不可收集的物品......

在进行项目设计时，将机器人的行为本质上划分为两个任务：接近可收集的物体、决定是否应该收集它。到目前为止，agent已经设法自己弄清楚这两项任务。但只是为了好玩（或者为了加快训练过程），我们可以“硬编码”第二个目标。关于激活抓取机构的决策只是一个简单函数的输出，考虑两个方面的因素：

1. 面前的对象类型（由语义分割图定义）

2. 物体与agent的距离（由深度图提供）
...
如果是yes，那么可收集物体就足够接近agent，因此就可以收集它！

<img src="images/heurgif.gif" width="300">

启发式背后的决策。

4 总结和可能的改进

我们使用几种不同的方法在定制环境中创建了自主机器人的模拟。为了将原型转变为可以提供商业价值的实际产品，还应该引入一些改进：

**安装垃圾收集的实际机构** - 如前所述，应该详细模拟机器人的机械设计。这意味着安装一个“铲子”，可以无缝地将垃圾推入机器人的“腹部”。因此，一旦机器人的容器装满，我们还应该设计一种巧妙有效的方法来处理这些物品。
